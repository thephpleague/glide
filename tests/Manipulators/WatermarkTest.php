...
class WatermarkTest extends TestCase
{
    ...
    public function testRun()
    {
        $image = Mockery::mock('Intervention\Image\Image', function ($mock) {
            ...
            $mock->shouldReceive('getDriver')
                ->andReturn(Mockery::mock('Intervention\Image\AbstractDriver', function ($mock) {
    ...
    public function testGetFit()
    {
        ...
        $this->assertSame(
            'crop-bottom',
            $this->manipulator->setParams(['markfit' => 'crop-bottom'])->getFit()
        );
        $this->assertSame(
            'crop-bottom-right',
            $this->manipulator->setParams(['markfit' => 'crop-bottom-right'])->getFit()
        );
        $this->assertNull($this->manipulator->setParams(['markfit' => null])->getFit());
        $this->assertNull($this->manipulator->setParams(['markfit' => 'invalid'])->getFit());
    }
